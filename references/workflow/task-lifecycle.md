...
- **`done`** — set by `implement-task` at finalization, alongside a closing `**Completed:** YYYY-MM-DD` line; a reconciler's verified advance writes both the same way (`./status-transitions.md`).
