...
- **`done`** — set by `implement-task` when the acceptance gate passes with every goal satisfied — reached from `executing` (no external goals) or from `in-review` (the external goals are now confirmed and the later run's fresh integrated-health boundary passed). Implies a companion result file carrying the gate's `## Acceptance` section and a closing `**Completed:** YYYY-MM-DD` line. Also entered by a reconciler's verified advance through the shared engine, in either direction, registered in `./status-transitions.md`.
