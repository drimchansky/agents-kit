...
- **the kind** — `pr` or `commit` — which selects the kind-specific headings of § *The return*;
...
2. **Completeness.** Every § *The return* heading is present — an explicit `None` counts, an absent heading does not. A return missing one is malformed: a matching digest with findings but no `Verification scripts` would render `delegated (<model>)` with the always-run guarantee silently unmet. Four headings may never be `None`, and a `None` under one of them is malformed exactly as an absent heading is: `Identity`, `Verification scripts`, and `Change map` on every pass — the script heading lists each exposed script with its outcome, and a project exposing none says so as a listing (`none exposed`), never as `None`, or the check meant to catch that unmet guarantee passes on it, and a `None` map leaves the session with nothing to draft from and the lens fleet with no set to derive — plus `Summary` on the `pr` kind. Take § *Degrade rule*'s path with reason `reviewer failed`.
