...
- **the review object, named concretely, with its identity** — never pasted diff text. For the `pr` kind, the diff `<base>...HEAD`, with the reviewed head SHA and the merge-base SHA; for the `commit` kind, the staged diff (`git diff --cached`), with its `git diff --cached | git hash-object --stdin` digest. The reviewer assembles the object itself;
...
- **the kind** — `pr` or `commit` — and its kind-specific ask, per § *The return*;
- **the PR context** for the `pr` kind — title, description, review comments and discussion threads, and every URL extracted from them. Links travel as URLs, not as fetched content: the reviewer fetches them itself and records what it could not reach;
- **the flags in effect that change the return** — `-d` (the drafted PR description) and `-p` (the compact change map);
...
- **the host skill's § *Review Focus*, verbatim** — the kind-specific emphasis the skill owns (what to read first, what to prioritize), which the review pack does not carry. The session composes it into the packet at launch from the skill it already holds, so nothing is copied on disk and nothing needs mirroring.
...
Return evidence, not a verdict. Include every heading, using an explicit `None` where one is empty — except the headings § *The settle*'s intake check 2 names as never empty, where a `None` fails that check:

- `Summary` — the `pr` kind only: what changed, its intent, and the reviewer's overall assessment (approve / request changes / needs discussion). Evidence like the rest — the session restates the assessment after settling, but only the reviewer read the diff, so the summary's substance comes from here.
...
- `Improvements` — the `pr` kind only: non-blocking suggestions. Not findings — they carry no required `file:line`, and the settle passes them through rather than dropping them as uncited.
- `Identity` — the object's identity echoed back as the reviewer resolved it at the effective root: the head and merge-base SHAs, or the staged-set digest.
...
- `Commit message` — the `commit` kind only: the drafted message for the reviewed set.
- `PR description` — the `pr` kind under `-d` only: the ready-to-paste body.
...
- `Change map` — only when `-p` was requested: the reviewed set grouped by file and intent, compact enough to read at a glance and complete enough for the session to derive its lens set from.
...
1. **Identity.** The `Identity` echo matches the identity the host skill's Setup recorded — the head and merge-base SHAs, or the staged-set digest. A mismatch means the reviewer resolved a different object: stop and report; settle nothing, launch nothing.
2. **Completeness.** Every § *The return* heading is present — an explicit `None` counts, an absent heading does not. A return missing one is malformed: a matching digest with findings but no `Verification scripts` would render `delegated (<model>)` with the always-run guarantee silently unmet. Five headings may never be `None`, and a `None` under one of them is malformed exactly as an absent heading is: `Identity` and `Verification scripts` on every pass — the script heading lists each exposed script with its outcome, and a project exposing none says so as a listing (`none exposed`), never as `None`, or the check meant to catch that unmet guarantee passes on it — `Summary` on the `pr` kind, `Commit message` on the `commit` kind, and `Change map` whenever `-p` was requested, where a `None` leaves the lens fleet with no map and no instruction. Take § *Degrade rule*'s path with reason `reviewer failed`.
...
Session-facing. The skills authorized to launch or consume a delegated reviewer — the membership § *Launch packet*'s gate checks a packet against: `review-commit` and `review-pr` launch one; the two composites — `review-commit-triage-verify` and `review-pr-triage-verify` — consume this contract through their phase 1's execution of the sibling review skill. Authorization is contract content, the same class as `./executor-contract.md` § *Bindings* — not a record of who cites this file, which reverse search reconstructs.
