...
1. **Run the report first.** `node <kit-root>/scripts/task-state.ts <task-dir>` — `<kit-root>` and the script's own contract section both per `./task-store.md` § *Resolving `<kit-root>`*. It carries the plan's status, every step in plan order with its checkbox, `nextPendingStep` with its `**What:**` and `**Verify:**` lines, each checkpoint's `**Outcome:**`, whether a checked step's `([result](…))` anchor still resolves, goal coverage, and `result.md`'s `currentState` block. Its exit 1 is a folder holding no readable `plan.md`.
