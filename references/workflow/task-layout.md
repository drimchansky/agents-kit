...
A directory tree that groups many task folders — a task **store**, like a central `Tasks/` repo with area subdirectories — may carry store-level files. All three kinds below are optional; skills detect them by existence and degrade silently when absent.
