...
A directory tree that groups many task folders — a task **store**, like a central `Tasks/` repo with area subdirectories — may carry store-level files. All four kinds below are optional; skills detect them by existence and degrade silently when absent.
...
- **`.maintain-last-run` — the maintenance ritual's run marker.** The store root may carry a `.maintain-last-run`: a single ISO `YYYY-MM-DD` line naming when `maintain` last swept the session logs, read by that skill as its analysis window's start and rewritten with the run date once the sweep finishes. Derived run metadata owned by `maintain` alone — not task content, and no other skill reads or writes it. Like the index regeneration above, the rewrite is a sanctioned unprompted write; absent → `maintain` falls back to a 30-day window and creates the file on its next completed sweep.
- **`DOC_CONVENTIONS.md` — org documentation conventions.** The store root, or an area within it, may carry a `DOC_CONVENTIONS.md`: the single home for the org-specific documentation conventions the kit's format checklists deliberately exclude — people/mention tables, house style, page-handling conventions for published docs. Discovered by **walk-up**: from the task folder, check each ancestor directory up to the store root; the nearest file wins (an area-level file overrides a store-root one). Absent → only the kit's generic format bars apply. Consumed by documentation-domain work — the pack checklists and the `stage-doc` skill cite this role rather than hardcoding a path.
