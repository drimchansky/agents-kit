...
The cross-vendor engines the opt-in `-x` cross-check runs on — each one's launch recipe, what it enforces, and what it costs — split out of `./probe-engines.md`, which keeps the native engine every other fan-out uses. The probe contract that binds every engine is `./agent-fanout.md`; the `-x` contract that selects between these two is `./probe-cross-check.md`. Read this file only when an `-x` run is launching its probe.
