...
**The entry gate is *unstarted*, and `scripts/task-move.ts` applies it.** Which folder states it admits and which it refuses is that script's contract — `<kit-root>/SCRIPTS.md` § *`scripts/task-move.ts`* owns it, read against the status vocabulary in `./task-lifecycle.md` § *Status values*. A task the gate refuses is not parked by hand either: a live one pauses through the `blocked` status, never by being moved, and a finished one archives (`./task-archiving.md`) — including one already misfiled inside a `Backlog/`, which that file's backlog exception takes **out** of the backlog rather than into a nested `Backlog/Archive/`.
