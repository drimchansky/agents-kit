...
3. **`CONTEXT.md` seed** — the full schema skeleton per `context-schema.md`, every section heading present, placeholders where nothing is decided:
    - `**Domain:**` inferred per part — and when a part is clearly non-code with no clear domain, asked in the confirmation round rather than stamped wrong (`context-schema.md`'s field note).
    - `## Problem Statement` cites `./ticket.md` — never restates the ask.
    - `## References` carries the **source pointer** (the source task's slug and the doc's role name, a root-relative path for a store-level doc, or a URL — per `./one-home.md` § *One home per fact*; for a pasted source, a dated *pasted into session* note — the facts carried below are then the part's only durable copy), the part's **Jira key** when mapped, and the **shared facts** from the source that this part needs. Those split by where the fact already has a home: one an applicable ancestor `GROUP_CONTEXT.md` holds is cited there by its root-relative path and left where it is (`./task-store.md` § *Shared group context*), so the group stays its single home for every sibling at once; one no such group holds is duplicated into the folder, because siblings share no layer of their own (`./one-home.md` § *One home per fact*, closing rule).
...
4. **Handoff** — the run's report ends with `Next: /plan-task <first-part>` — the handoff token `task-layout.md` § *One task, one flat folder* fixes for where the parts landed: the bare slug where one resolves, the folder's path otherwise — plus one line per remaining part.
