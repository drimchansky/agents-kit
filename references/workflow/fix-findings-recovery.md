...
A delegated fix runs through an **executor** per the `fix-findings` binding in `./executor-contract.md` § *Bindings* — read it before the first delegation — on the engine and defaults in that file's § *Write-mode engine registry*. Unflagged that engine is `native`, whose host adapter supplies those defaults; with `-x` (`fix-findings` § *Flags*) it is that registry's `cross` entry instead — no adapter loads there, so the packet carries what one would have supplied — for exactly the fixes the posture procedure already comes out `delegate` for and inside the same Confirmed auto-path surface (`fix-findings` § *Execution strategy: inline by default*) — a fix it comes out `inline` for still runs on the coordinator, flag or no flag. Under `-x` every delegated fix runs in a coordinator-managed worktree, a serially delegated one included, per that entry's worktree-always placement. That binding fixes the packet; the point of it is that the executor sees only the packet — the finding verbatim, its root cause, the chosen fix option, the expected surface, its processing order and any known dependencies, the always-applying pack section verbatim, the engineering-pack guidance the fix's surface triggers by path — and never this session, so whatever the fix depends on has to be in it. Its immediate-outcome result is advance evidence: the coordinator captures the ordered change set and performs the final-integrated checks and health boundary.
...
**Announce and record.** Say in chat which fixes are being delegated and why the procedure came out delegate, and note the delegation and the engine that ran it inside the affected `Fixed` / `Fix failed` entry — no new bucket. That record is what keeps the default from drifting silently either way.
