...
A delegated fix runs through an **executor** per the `fix-findings` binding in `./executor-contract.md` § *Bindings* — read it before the first delegation — using the native engine and host adapter defaults in `./agent-fanout.md` § *Write-mode engine registry*. That binding fixes the packet; the point of it is that the executor sees only the packet — the finding verbatim, its root cause, the chosen fix option, the expected surface, its processing order and any known dependencies, the always-applying pack section verbatim, the engineering-pack guidance the fix's surface triggers by path — and never this session, so whatever the fix depends on has to be in it. Its immediate-outcome result is advance evidence: the coordinator captures the ordered change set and performs the final-integrated checks and health boundary.
...
**Announce and record.** Say in chat which fixes are being delegated and why the procedure came out delegate, and note the delegation inside the affected `Fixed` / `Fix failed` entry — no new bucket. That record is what keeps the default from drifting silently either way.
