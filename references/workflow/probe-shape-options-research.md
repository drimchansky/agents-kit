...
The prompt shape for `review-task`'s question research, one probe per open question the assessment derived. The probe contract and the merge contract that bind it are `./agent-fanout.md`; the engine is `./probe-engines.md`, always `native` with no cross-vendor hop. Questions about the same decision or the same file batch into one prompt. Answers merge under `./agent-fanout.md` § *Merge contract* and close on the consuming skill's line at the end of its Questions output: `Question research: <n> answered, <m> optioned, <k> no-evidence`. The line renders only when the review surfaced candidate questions; its three counts are per question. Inline fallbacks append, in this order: `, <j> researched inline; skipped (<reasons>)` when no engine was available or a probe died (several questions' reasons comma-join inside the one parenthesis), then `, <r> researched inline (classification rejected)` when the session's re-check rejected a probe answer that ran. The two counts are disjoint.

This shape reads whole files and compares across them, so a Plan-type agent is the most restricted fit; an excerpt-reading Explore-type starves the comparison (`./probe-engines.md`).
