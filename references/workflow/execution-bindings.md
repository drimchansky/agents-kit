...
- **Record**: the chat report; no task bookkeeping or status edit. A task-folder document changes only when selected as a finding's work product
- **Blocked**: an **immediate**-outcome failure restores that fix's pre-fix capture and continues with the next finding; a pending impactful choice restores the same way and lists the finding Awaiting decision (`fix-findings` § *Content baseline and immediate outcomes*); a failed final-integrated outcome goes to the skill's dependency-safe recovery, which may revert a whole implicated group or, unconverged, restore the pre-run baseline and stop. Never leave a failing fix or implicated group in the tree. A failed health boundary isolates fixes only when the failed check is green at the baseline; a matching baseline failure retains the survivors as **Health uncertifiable**; an inconclusive comparison restores the pre-run baseline and buckets every retained fix as Fix failed (`fix-findings` § *Integrated health boundary*)
- **Acceptance**: every selected finding in exactly one report bucket, re-read against the live tree
- **Health boundaries**: no up-front and no per-batch health run. One boundary on the retained-fix tree after every retained fix's final-integrated outcome passes; it runs the engineering recipe for changed code and the documentation recipe for changed prose. Every tree-changing recovery earns one fresh boundary before any survivor is Fixed (`fix-findings` § *Integrated health boundary*).
