...
- **Health boundaries**: no up-front, per-finding, or per-parallel-batch health run. One boundary per concern batch on its retained-fix tree, after every retained fix's final-integrated outcome passes and before the batch commits; it runs the engineering recipe for changed code and the documentation recipe for changed prose. Every tree-changing recovery earns one fresh boundary before any survivor is Fixed (`fix-findings` § *Integrated health boundary*).
