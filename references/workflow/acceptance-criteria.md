...
Quality bar for the bullets in `spec.md`. A criterion that fails any check below is not ready for `review-task` or `implement-task` — fix it (or mark `_(unresolved: <note>)_`) before downstream skills run.
