...
- **Docs → reality.** A composite whose first phase reports on a task folder and whose second, after that report is printed, writes its findings back so the docs stop *overstating* what's been built. Two composites reconcile in this direction today: `resume-task-reconcile` (its brief's drift findings) and `review-task-reconcile` (its review findings, plus the engineer answers it folds into the plan). The base skills they run — `resume-task` and `review-task` — are strictly read-only on their own.
...
Both composites apply these; only the evidence source differs (the resume brief's drift check against disk, the review's cross-file consistency check). **auto** = obvious, applied unprompted; **ask** = engineer input first.
...
- **External blocker cleared** (PR merged, ticket closed) — no status flip — `blocked` clears when work resumes, and reconciliation doesn't resume work. Record the observation in the Reconciliation entry when other edits already warrant one (never append an entry just for it); either way "Not reconciled" names `implement-task`.
