...
Some skills write a task folder's docs back into agreement with reality after the fact. There are two reconciliation **directions**, and this file is the **single source of truth for the mechanics they share** — consent model, reference sweep, record and annotation formats, `plan.md` openings, run sequence, and the verify-and-write engine every state advance goes through. Each direction's write surface, edit rules, and finding-type → edit mappings live in its own file; the skills cite these files rather than restating them.
...
- **Session → docs** (`./reconciliation-session-to-docs.md`) — `reconcile-task`, which reviews the current session against the docs and writes back what surfaced in conversation but never reached the folder, and the composite `review-pr-triage-verify-reconcile`, whose reconcile phase writes back the Confirmed findings its review phase produced: the *enriching* direction. Neither gates its write behind a flag — writing back is the whole purpose of `reconcile-task` and of the composite's reconcile phase, so both always write.
...
Reconciliation fixes the **docs, not the world**: no code changes, no git mutation, no writes to external systems. Every edit maps to a finding printed in the report, or to an engineer's answer about one; drop anything else. Two run-record writes are the registered exceptions, each licensed where it is defined rather than by a finding: the sweep's `observations.md` rewrite (§ *External reference check*) and the commit watermark's seed, re-seed, and advance (`./reconciliation-commits.md` § *Degenerate cases* for the first two, § *The record* for the advance). (A reconciler may *run* verification to back a state change, but running a check is read-only — no code changes, nothing outside the task docs mutates.)
...
The **reference sweep** re-derives the freshness of the external systems a task folder cites. A reconciler that derives its finding set from the folder or the session runs the sweep while assembling that set, and the sweep is the **only** re-deriver of cited-reference freshness (`./one-home.md` § *One home per fact*, external-system facts).

**Exempt — a finding set pinned to a producing phase.** A reconciler whose finding set is fixed by a producing phase never reads the folder's reference list to assemble it, so a sweep would inform nothing it writes: it runs none, and renders no `## References` block. `review-pr-triage-verify-reconcile` is the one member on that footing — its reconcile phase writes the Confirmed findings its review phase produced and nothing else.
...
Every reconciler that writes `result.md` closes by rewriting its `## Current state` block to post-edit reality — ≤1 KB, superseded detail dropped; history lives in the log. A legacy result without the block gains one here (per `./task-authorship.md`, created at the next write). By the result's state when the run began:

- **Live (`executing`/`blocked`/`in-review`) — full rewrite:** `_Updated: YYYY-MM-DD_`, a one-line gloss consistent with the (possibly just-changed) `**Status:**`, `**Pointers:**` refreshed to the identifiers in play, `**Next:**` naming the concrete next action. The `**Pointers:**` rewrite carries the commit watermark forward untouched, as it binds every writer of that field (`./task-authorship.md`, the `**Pointers:**` clause); a reconciler is additionally the only thing that *advances* it, and only where `./reconciliation-commits.md` says. The block may never claim a stronger lifecycle state than `**Status:**`. In either direction it is re-derivation of a digest constrained by that `**Status:**`, never a claim of its own: `**Status:**` and the checkboxes move under the direction's own rules — a downward repair, or an advance through § *Strengthen only on verified evidence* — and the digest follows wherever the run left them.
- **Already `done` when the run began — partial freeze:** the narrative half, gloss and `**Next:**`, stays frozen as the final digest (`./task-authorship.md`: that half is expected on *live* results only), so a finding against a completed task is never recorded by re-glossing it.
...
At the end of a `reconcile-task` run, if `result.md` exceeds **20 KB**, add a judgment item to the batched round proposing compaction — never auto-apply it, and docs → reality never compacts. Compaction is the one sanctioned removal of prior log sections, safe only because the removed text stays recoverable. The procedure — the `HEAD`-resolvability precondition, what collapses and what always stays, and the tombstone format — is `./reconciliation-compaction.md`; read it when the trigger fires.
