...
**Exempt — a finding set pinned to a producing phase.** A reconciler whose finding set is fixed by a producing phase never reads the folder's reference list to assemble it, so a sweep would inform nothing it writes: it runs none, and renders no `## References` block. `review-pr-triage-verify-reconcile` is the one member on that footing — its reconcile phase writes the Confirmed findings its review phase produced and nothing else. The exemption transfers no ownership: `reconcile-task` stays this direction's sweep owner and still sweeps the same folder, so the folder's citations keep being swept and the sweep stays their only re-deriver.
