...
3. Whatever the run leaves unwritten goes to the "Not reconciled" list, each line opening with one of three labels — **this section is the one home for the labels**; the direction files and the skills cite it rather than restating it:
   ...
   - **Yours to apply** — a surface no reconciler writes (`ticket.md`, a deliverable's `**Published:**` line, an applicable `GROUP_CONTEXT.md` — § *Never-annotated surfaces*), the proposed text attached where one exists, so the repair is a paste. A mapping row saying "Not reconciled" names the user lands here.
...
**A fourth surface joins them by inheritance rather than through the sweep: an applicable `GROUP_CONTEXT.md`.** The shared grounding a task sits under (`./task-store.md` § *Shared group context*) is read by every reconciler and written by none. It reaches a run off disk rather than through an enumerated citation, so it passes no inclusion test and offers the sweep nothing to fetch: a URL sitting inside a group file is not swept, not annotated, and not dated by anything here. A correction whose home is that file is **flag only** on the four terms above, the owner named being that file's owner, and it lands in *Yours to apply* with the proposed wording attached. An open question the session answered whose home is a group file takes this rule as well, never the answered-question annotation of § *Annotation formats*. What a reconciler may still propose is the **task-local consequence** — a step's scope or its `**Verify:**` line, a `CONTEXT.md` prose section, a goal — each on the surface that owns it, through its direction's **ask** row and § *Grounding docs change by confirmation, never silently*, never auto and never as a status flip on its own.

## Flag-only findings are re-reported

A finding routed **flag only** — every finding on the four surfaces above, and every mapping row marked so — is re-reported *with its finding*, never reduced to its ledger line, until the owner named with it records the repair. Where a sweep tag carries the finding, re-reporting reaches exactly as far as that tag does, and no further: **every** sweep on a fetched `block`, **every** sweep on a carried `warn`/`block`, but **once** for a `warn` that a later clean fetch re-tags `info` (`./reconciliation-sweep.md` § *Tags*), past which no observation carries it. A finding carrying no tag at all — what the unswept `GROUP_CONTEXT.md` surface produces — has nothing that could retire it, so it is re-reported on **every** run that can still see it until that repair is recorded: a contradiction between the group file and the task's own artifacts is re-read off disk each run, and a finding noted in a `## Reconciliation` entry is re-read from there. A finding only the session established — an answered question, a constraint the session learned had changed — and that no entry carried (term 2 above writes none just for it) leaves nothing a later run can read, so it is reported in full on the run that found it, and the owner named with it holds the only record. **This section is the one home for the rule and fixes its one scope**; the direction files and `./reconciliation-sweep.md` cite it rather than restating either.
