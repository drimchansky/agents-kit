...
- **External blocker cleared** (PR merged, ticket closed) — the `blocked`-clearing case of the shared *Cited reference materially changed or gone* repair above: **apply that row in full**, including its `**Pointers:**` refresh. Its no-status-flip rule holds here for its own reason: `blocked` clears when work resumes, and reconciliation doesn't resume work. "Not reconciled" names `implement-task`.
...
- **Cited reference materially changed or gone** (reference check, `warn` — PR merged, ticket closed, doc rewritten — or `block` on a `**Pointers:**` entry) → **auto** for the world-truth surfaces only: refresh the affected `**Pointers:**` entry in the *Current state refresh* — for a `block`, a dated gone/moved note on the entry, keeping the identifier — and note the observation in the `## Reconciliation` entry when other edits already warrant one. Never a status flip on its own — a merged PR is not the acceptance gate, and `blocked` clears when work resumes; "Not reconciled" names `implement-task`. A changed reference that contradicts `CONTEXT.md` prose is an **ask**, per the grounding-docs rule. (A `block` on a `CONTEXT.md` or `plan.md` citing line stays with the *Cited link broken* row above.)
