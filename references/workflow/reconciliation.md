...
- Each removed section becomes one line under a single `## Compacted — YYYY-MM-DD` stub naming the collapsed anchors. Each bullet is the removed heading's text and nothing else — `- Step 1 — Old title` — because `scripts/health-check.mjs` resolves a step's evidence link by slugifying the whole bullet line, so any word added to one stops the tombstone answering the anchor it names. The stub then closes with "full text in git history (pre-compaction state)." as its own line after the bullets, never appended to one.
