...
`ticket.md` states the product-facing ask the goals derive from, and an applicable `GROUP_CONTEXT.md` (`./task-store.md` § *Shared group context*) states grounding shared by every task under it. Both are **writable** by either direction, as **judged** edits under the consent model, so a correction lands where it belongs instead of being handed back as a paste. An annotation § *Annotation formats* fixes — a broken-link note on a ticket citing line, an answered-question note in a group file — keeps the route that section gives it on either surface, owing only term 1 below; **judged** is the route for a content edit. Both reach past this task folder, which fixes what a write to them owes:
...
- ticket.md — Acceptance criteria: added the row-cap criterion the requester stated in the linked issue on 2026-09-02 — external surface: `ticket.md` § Acceptance criteria; was "any table exports", now "tables up to 50k rows export" — finding: ticket ↔ goals drift — chose adding the criterion over rewording the existing one: the requester's comment bounds the ask rather than replacing it
...
- a `block` on a `CONTEXT.md`, `plan.md`, or `ticket.md` citing line takes the broken-link format of § *Annotation formats*, not this row; a `warn`/`block` on a never-annotated surface takes § *Never-annotated surfaces* rather than either;
- a `warn` on a `ticket.md` citing line is the one place the sweep can show the *ask itself* moved: where the fetched source states a criterion the ticket lacks or words one differently, the ticket edit is **judged**, on the terms of § *The upstream ask is writable, and never rewritten quietly* — the requester's wording is the evidence, both wordings are quoted, and `plan-task` is named under "Not reconciled" per that section's term 4; a `warn` that leaves the ask as stated routes no ticket edit, and is noted in the `## Reconciliation` entry only when other edits already warrant one;
