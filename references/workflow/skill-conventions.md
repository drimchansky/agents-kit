...
- `watch-pr-author` — verify what the reviewers wrote on a pull request (`triage-findings-verify`, handed the pull request explicitly), then apply the fixes that triage confirms (`fix-findings`, handed the named Confirmed subset explicitly), with a session-held quiet check ahead of the dispatch: a repeat pass in one session against a pull request whose `headRefOid` and `updatedAt` have not moved reports no change and dispatches nothing, the comparison living in session memory alone and never on disk. Composite rather than a flag on either dispatched skill: each phase finishes and prints before the next begins — the verified triage's verdicts are the fix phase's entire input — so there is a clean seam at the join and both dispatched skills stay whole and unaware, each handed its source explicitly rather than resolving one of its own. Its write surface is the working-tree code `fix-findings` edits and nothing else: no task folder, nothing staged, nothing committed, and nothing posted back to the pull request — the pass stops at a commit-ready tree.
- `watch-pr-review` — resolve the git worktree holding a pull request's head branch (prune it on close, reuse it wherever an exact branch match puts it, otherwise create it beside the main checkout), then run the verified review (`review-pr-triage-verify`) anchored in that worktree, with the same session-held quiet check ahead of phase 1, binding open pull requests only so a closed one always reaches the prune — even the retry of a prune Git refused: a repeat pass in one session against an open pull request whose `headRefOid` and `updatedAt` have not moved reports no change and resolves nothing, the comparison living in session memory alone and never on disk. Composite rather than a flag on the dispatched pipeline: the worktree is fully resolved and printed before the review begins, so there is a clean seam at the join and that pipeline runs whole and unaware at a repository root it is simply handed — a flag would instead have threaded that root through every phase of it. Its write surface is worktree lifecycle alone — `git worktree add`, a `--ff-only` refresh, a lockfile-keyed dependency install after that refresh, on creation and again when a fast-forward moved a lockfile, and the close-time `git worktree remove` that never passes `--force` — so it edits no source file, writes no task folder, stages nothing, commits nothing, and posts nothing back to the pull request; the verdicts land in the session, where `/publish-pr-review`'s counted tier picker is the one path by which anything reaches the pull request. That is also why it stays model-invocable, on § *The invocation gate*'s two closing criteria rather than on where the pass leaves what it makes: it is no terminal filing or publishing step — the one outward-facing act, anything reaching the pull request, stays behind that picker — and its run reaches past the current project neither way, across no registered root and into no installed state, the worktree it adds being one more checkout of this same repository beside the one the engineer invoked from and listed by `git worktree list`, not the sweep the second criterion closes the door on. The removal above never passing `--force` is what keeps an unasked run from discarding anything of the engineer's — no roster entry, no `agents/openai.yaml`.
