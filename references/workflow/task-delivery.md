...
Run assertions and integrated health before staging. A failed gate records failure and no commit, then Stop-the-Line. After a green gate, stage only attributable task work since the prior checkpoint, using explicit paths or hunks. Inspect the complete staged diff and verified bytes. Unrelated staged content or ambiguous overlap blocks the commit; never absorb, reset, or stash it. Then run `skills/commit/SKILL.md` in ordinary mode over that staged set, passing as its argument the why of the steps the checkpoint bounds, from their plan entries. The full-plan request above is the explicit permission its opening paragraph requires. Never pass `--amend`. Its message rules, guard scan, verification gate, signing handling, and report apply unchanged. Name the checkpoint's green boundary in that argument; `commit`'s gate reuses it when the index is the verified tree.
