...
`observations.md` is an **optional** derived role file holding the last observed state of every external reference the task folder cites **from an actionable surface** (`./reconciliation-sweep.md` § *Scope*, whose enumeration `scripts/sweep-scope.ts` performs — a link the report leaves out has no line here) — one dated line per URL, written by the reconcilers' reference sweep and rewritten **wholesale** on each sweep (`./reconciliation-sweep.md` § *Ledger*); nothing in it is appended or hand-maintained. A folder may have one or not, and **absence is never a gap**: it means no sweep has run yet, or nothing is cited from an actionable surface. There is no `**Status:**` field — the `_Swept:_` line carries currency — and the file sits outside the companion-result-file rule (`./task-lifecycle.md`), the same footing as `diagram.md`.
