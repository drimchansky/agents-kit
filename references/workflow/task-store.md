...
- **A script's own contract is its `SCRIPTS.md` section.** `<kit-root>/SCRIPTS.md` § *`scripts/<name>.ts`* owns each script's CLI form and its stdout contract, and a citing skill reads that one section rather than the whole file. This bullet is that idiom's home as well, so a skill names its script's section without spelling the rule out again.
