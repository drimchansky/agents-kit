...
**Where the recipe would run an infra-bound command over the whole tree, run it at CI's scope instead**, since CI gates the merge at that scope. The whole tree stays where a § *Widening* trigger the runner's graph cannot see fires: a tool config outside the runner's declared inputs, a codegen input, or an edge no static import graph contains. CI's selection shares that blindness, so its scope would drop the guard the trigger exists for. A boundary with a delta has checked those triggers against it. Where no delta chose the whole tree, check them over the paths CI's selection would read, `git diff --name-only <merge-base>` and `git ls-files --others --exclude-standard`, before taking CI's scope; record a firing trigger as the widening. CI's scope is the project's CI command for it when that command selects against a base, such as `nx affected -t test:e2e`; run it from the checkout. The run must select from the merge base with the target base through the working tree, uncommitted and untracked files included. Remove every revision pin that narrows that selection. For Nx, drop CI's `--base`, `--head`, `--files`, `--uncommitted`, and `--untracked`, and unset `NX_BASE` and `NX_HEAD`, since a set head limits `nx affected` to committed files. Without them, it diffs from the merge base with the workspace's `defaultBase` and includes uncommitted and untracked files. Pass `--base=<branch>` when the branch targets another base. The whole tree stays where `git merge-base --is-ancestor HEAD <base>` exits 0 and where the checked-out branch is the base itself: the selection then cannot tell the branch's own commits from the base's and would drop any. A branch at the base's tip keeps CI's scope only when it has no commits of its own: the last line of `git reflog show <branch>` is its `branch: Created from` entry and `git rev-list --count <that entry's SHA>..HEAD` prints 0. CI's scope then selects its uncommitted and untracked work. Without that entry, or with a non-zero count, the whole tree stays. The whole tree also stays where a runner's documented behavior does not establish the merge-base selection, as with turbo's `--filter=...[<ref>]` and `--affected`. The same holds where CI runs the command over the whole tree, or not at all. Record the scope as `CI scope: <command>`, naming the command as run, and add `no tasks selected` when it selected none.
