...
**Inherited grounding against what the task asserts.** The shared grounding §2's read order loads comes off disk on every invocation, from wherever the folder sits *now* — so a group file edited since the plan was written, or a task moved between groups, lands in this check rather than waiting for someone to remember a refresh. Weigh it against the `- [ ]` steps and against the claims partitioned above alike: where an applicable group file materially contradicts `ticket.md`'s scope, `CONTEXT.md`'s recommended direction or a key assumption, what a pending step is about to do, or what a shipped claim or a `met` goal says was already done, that is drift. Quote both statements and name both sources — the group file by its path from the selected root, the step by its number or the goal by its `G<n>` ID, and the ticket or `CONTEXT.md` by its section — and tag it `warn`, or `block` where a pending step cannot be run as written. Re-reading a chain invalidates nothing by itself: one that contradicts nothing the task asserts produces no finding at all, and either way this skill proposes no edit to the group file or the plan (`./references/workflow/task-store.md` § *Shared group context*).
...
- [block] Step 4 (pending) deploys to region `US`, while `product/backend/GROUP_CONTEXT.md` (root `Tasks/`) now reads "services deploy to region EU only" — settle which holds before running the step
