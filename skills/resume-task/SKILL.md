...
- `observations.md` (when present) — the last sweep's dated observations of the folder's cited references (`./references/workflow/task-layout.md` § *The observations file*); a cache to quote with its dates, never live truth. Absence is normal — no sweep has run, or nothing is cited.
