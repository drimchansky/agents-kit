...
- Walk each search directory exactly as `./references/workflow/task-store.md` § *The root registry* walks a registered root — read that rule at run time; it owns the whole test. The **recognition set** that walk applies (`./references/workflow/task-layout.md` § *One task, one flat folder*) includes the legacy suffix forms, which matter most here: a root whose folders still carry those forms is exactly the kind this walk must not miss.
- A task folder under `<x>/.agents/tasks/` belongs to the **canonical root** `<x>/.agents/tasks` — propose that, not the project directory.
- Otherwise keep climbing while **either** condition holds, and propose the highest ancestor that satisfies one. **Every sibling directory also leads to a task folder** — or carries a `GROUP_CONTEXT.md`, which counts the same, being a group waiting for its first task and part of the store (`./references/workflow/task-store.md` § *Shared group context*). **Or the ancestor itself carries a `GROUP_CONTEXT.md`**, whatever its siblings hold: a root proposed beneath it would leave that group file above every root and outside every task's chain. The two are separate tests because they answer separate questions — the first is what separates a store (`Tasks/` holding `area/task/`) from a directory that merely happens to contain one, and the second is what keeps the store's own shared grounding inside the root it grounds, which a sibling test alone does not reach: one ordinary sibling holding neither tasks nor a group file would otherwise stop the climb one level below the group file and strand it. Climb out of any `Archive/` or `Backlog/` container first — each is a container, not a root (`./references/workflow/task-archiving.md`, `./references/workflow/task-backlog.md`).
- Beyond that walk's own prunes, skip a `tests/fixtures/` tree: it is this skill's one addition — fixture task folders are test data, and proposing them as roots is the most likely way this skill produces a wrong file.
