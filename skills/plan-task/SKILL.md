...
**`goals.md`** — the contract for what "done" means: a `## Goals` list of durably-ID'd `G<n>` acceptance criteria, no description prose, and **no `**Status:**` field**, being a static input rather than a lifecycle artifact. The user may hand-author it and edits it freely between sessions; `review-task`, `implement-task`, and `resume-task` read it and never write to it. `reconcile-task` and `review-task-reconcile` may add, reword, or retire a goal, only through a confirmed judgment item (`./references/workflow/task-authorship.md`).
