...
- **Integration assertions**: the plan's `### Checkpoint after Step N` headings, each mandatory after marking step N done. A checkpoint is not a step, carries no `- [ ]`, and is never flipped. Run its assertions, then its health boundary. When §2 granted checkpoint commits, the coordinator then applies `./references/workflow/task-delivery.md` § *Checkpoint commits* from the shared tree, never an executor worktree, running `commit` as that section directs. Append the checkpoint section (§5) after that attempt; in step-by-step mode no checkpoint commit is sanctioned, so append and pause. <!-- cold -->
