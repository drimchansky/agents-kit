...
When implementation reveals the plan is wrong — a step infeasible, scope wrong, a new step needed, or a step too large to land in one slice — apply the scope-change rules in `./references/workflow/execution-recovery.md` § *Scope changes mid-execution*, including its splitting strategies. The binding — the in-place plan update, the `**Deviations from plan:**` record, the structural repaint, the step-by-step pause, and the abandon flow — is `./references/workflow/implement-task-edges.md` § *Plan revisions* — read it when a revision is needed. <!-- cold -->
