...
- `-d` — Draft a ready-to-paste PR description (body only) in addition to the review, per the "PR description" section — drafted by the reviewer on a delegated pass and by the session on the inline fallback. Off by default.
...
- the flags in effect that change the return: `-d` and `-p`
...
Only when `-d` is passed. A ready-to-paste PR description — **body only, no title** — drafted from the change map and the links the review pass gathered: by the reviewer on a delegated pass, returned under `PR description`, and by the session on the inline fallback. Either way the session checks it against the rules below and prints it in a fenced block so the user can copy it verbatim.
...
Apply the Standard Verification Checklist in `./references/engineering/review.md`. The output carries the **Reviewed** provenance line (reviewed head SHA + merge-base SHA + reviewing model) and the mandatory `Review pass:` line. On a delegated pass: the packet named the same object and identity Setup resolved, both intake checks passed — the `Identity` echo matched that identity and every return heading was present — before any settle step or `-p` launch, the settle re-read every Critical and Major anchor before rendering it, the session ran nothing against the tree while the reviewer was in flight, and the line reads `delegated (<model>)`. On the inline fallback: the reason was announced and the line reads `inline (<reason>)`.
...
With `-d`: the drafted description is body-only and verdict-free, sources its links only from those already gathered — a `Task: <add ticket link>` placeholder rather than a fabricated URL — and carries no AI-attribution footer.
