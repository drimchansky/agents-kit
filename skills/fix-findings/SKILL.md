---
name: fix-findings
description: Use when asked to fix code or documentation findings from a session review, PR comments, or a pasted or saved list. Applies eligible Confirmed fixes automatically; other actionable fixes require batched diff approval. Edits local work products only, with no staging, commits, or source replies.
...
Edit selected working-tree work products only: no staging, commits, other Git mutations, edits to the findings report, source replies, thread resolution, pushes, or live-page writes. Transient coordinator-managed executor worktrees are the exception allowed by `./references/workflow/parallel-batch.md` § *Coordinator-side parallel batch*. <!-- cold -->
...
- **Record:** the chat report in § *Output*. Make no task bookkeeping or status edit. Edit a task-folder document only when it is a selected finding's work product.
...
When fixes remain, run one integrated boundary after their final outcome sweep. Run no up-front, per-finding, or per-batch health recipe. Use `./references/engineering/verification.md` § *Two verification tiers* for changed code. Use `./references/documentation/verification.md` § *Integrated health — declared boundaries* for changed documentation. Run both recipes for mixed work. The immutable pre-run baseline is the engineering reference and has no in-session green result, so run the whole relevant code surface. Sweep every retained local documentation deliverable and in-scope companion document for links and cross-references.

A finding is **Fixed** only when its final-integrated outcome passes and every applicable check in the unchanged final tree's health boundary is green.
...
**Next:** tell the user the fixes remain unstaged and unreviewed. They can stage them and run `/commit`; `/review-code` or `/review-code-triage-verify` reviews changed code, and `/review-docs` reviews changed documentation before merge or publication. Source replies, thread resolution, live-page updates, and pushes remain separate user actions.
