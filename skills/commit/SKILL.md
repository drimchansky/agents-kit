...
- **A review-commit message must exist in this conversation.** Find the commit message `/review-commit` drafted earlier in this session, or the one `/review-commit-triage-verify` forwarded — its Output carries the message and the **Reviewed** line in review-commit's own format, because its review phase *is* a `/review-commit` run. Either satisfies this precondition and every "review-commit" reference below. If there is none, stop and tell the user to run `/review-commit` first, then `/commit`. Do not draft a message yourself.
