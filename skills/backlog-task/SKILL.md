...
- **Unstarted tasks only.** A task parks only when it passes the **entry gate**, which the script applies and `./references/scripts/task-move.md` states. It is archiving's gate inverted, so don't carry `archive-task`'s reflexes into it — hand the folder over and let the gate answer. Never edit a `**Status:**` to make a task eligible, and never work around the refusal.
