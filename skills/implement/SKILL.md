...
- The user wants the work assessed rather than done — that's `review-code`
...
Close with `**Noticed but not touching:**` when applicable (`./AGENTS.md`), and point at the natural next step — `/commit` to commit the change and `/review-code` to review it before merge, or `/plan-task` if the work outgrew this skill.
