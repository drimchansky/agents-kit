...
description: Use when asked to publish review-pr's findings to the PR on GitHub — posts Major/Critical findings as inline comments, or a short approval if none. Follow-up to /review-pr only; the user provides no findings.
...
- [ ] Inline comments carry `Critical:` / `Major:` prefixes with correct `file:line`; findings off the changed lines listed in the body, not dropped
