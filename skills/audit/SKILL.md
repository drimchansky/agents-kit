...
Before working, read `./references/engineering/review.md` — it carries the lenses (What to Look For, What NOT to Flag, Calibrate Severity, Approval Bar, Prioritize Review Effort, Don't Rationalize) that apply to every audit. Its "Verification Scripts" section does **not** apply here — that recipe belongs to `review-commit` / `review-pr`, whose diff bounds what the scripts cover; an audit is analysis-only and runs no verification scripts.
