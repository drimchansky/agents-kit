...
Then read the resolved `plan.md`'s `**Status:**`, the companion `result.md`'s status, `goals.md`'s `G<n>` list (the round asks which goals a defect threatens), and `CONTEXT.md`'s `**Domain:**`, default `engineering` — the produced step's **Verify** field names that domain's health verify. Three outcomes stop the run:
...
- **`to-do`** — reviewed work sits on the branch beside a plan that was never started, which has no companion result file by design (`./references/workflow/task-lifecycle.md` § *Pairing rule*). The disagreement here is between the branch and the docs — whether the work began at all — not between a plan and its result file, so no reconciler repairs it and the missing file is not what stops the run. Surface it and stop; never write around it by appending to an unstarted plan.
- **No companion `result.md`** on a plan the two stops above left standing — `executing`, `blocked`, `in-review`, or `done`. A record is owed the moment a step is appended (`./references/workflow/reconciliation.md` § *The record*), and this composite creates no result file, so an absent one leaves the run owing a record it cannot write. On these four the absence *is* drift: `./references/workflow/task-lifecycle.md` § *Pairing rule* names it at its `executing` paragraph, and the docs → reality composites repair it — a skeleton result file where work is evidenced. Report that and stop.

A **`blocked`** plan is accepted rather than refused: appending steps enriches a paused plan and resumes nothing. It, and `executing`, `done`, and `in-review`, are all taken by `./references/workflow/reconciliation-session-to-docs.md` § *The target's status*, which decides what each one flips.
...
Confirm the protocol invariants before finishing. A `§` names a heading of `./references/workflow/reconciliation.md` or of its direction file `./references/workflow/reconciliation-session-to-docs.md`; a **bold** name is a paragraph inside the direction file's § *`review-pr-triage-verify-reconcile` — verified findings*. Check the behavior against the cited text, not against this list:
...
- [ ] No sweep run and no `## References` block rendered — § *External reference check*'s exemption for a finding set pinned to a producing phase
