...
Read the plan, the sibling `goals.md`, the sibling `CONTEXT.md`, **and** the `ticket.md` when present, in full before assessing anything. The plan is this skill's subject rather than its instructions, which is why it is read whole — the standing exception to `./references/workflow/task-layout.md` § *Reading a resolved folder*, the order the skills that execute or brief on a folder follow instead.
...
- **The shared grounding above the folder** — every applicable `GROUP_CONTEXT.md`, taken root-to-task, per `./references/workflow/task-store.md` § *Shared group context*, which owns the selection and the citation form. Reading the four files whole exempts this skill from the read *order*, not from what that order's third step loads, so the chain is read here explicitly and before anything is assessed; it comes off disk each run, from wherever the folder sits now. A folder no registered root contains inherits none, and nothing is read.

`CONTEXT.md` and `goals.md` are authoritative input for grounding, not optional — a task is expected to have both. `ticket.md` is authoritative when it exists; its absence is not a gap, since a task may legitimately have no ticket. If the goals file is missing, flag it as a gap up front — `plan-task` is expected to produce one and the acceptance coverage check below cannot run without it.
...
- **Inherited group context ↔ the task's own artifacts** — Does an applicable group constraint contradict the ticket's scope, a goal, CONTEXT's direction, or any step, the pending ones included? Report both statements with their sources, the group file cited by its path from the selected root, and stop there: the shared file belongs to its owner, so this review names only the task-local consequence and proposes no edit to prose above the folder. An empty or contradiction-free chain is not a finding.
- **Status field consistency** — Does the plan carry a valid lifecycle state, and does the companion result file carry what that state owes? The plan is the only file checked: a `**Status:**` line found in `CONTEXT.md` or `result.md` is legacy, neither validated nor flagged. An **absent** result file is part of this check, not an exemption from it: absence pairs only with a plan in `to-do` or `skipped`. Reject anything outside the vocabulary registered in `./references/workflow/task-lifecycle.md`.
...
- group context ↔ plan — drift: `product/backend/GROUP_CONTEXT.md` (root `Tasks/`) requires "services deploy to region EU only", while pending Step 4 deploys to `US`. Its owner decides the shared file; here, Step 4 is the consequence to resolve.
...
- [ ] Applicable group grounding read before assessing anything, and any contradiction with a task artifact reported under Cross-File Drift with both statements and both sources
