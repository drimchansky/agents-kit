...
- `-x` — Cross-check: launch one independent grounding probe on the cross-vendor engine and merge it into the grounding pass (Step 2), per the shared contract in `./references/workflow/agent-fanout.md`, with the engine and its launch recipe in `./references/workflow/probe-engines.md`. Off by default. The probe is read-only; its outcome is recorded on the Plan Summary's `Cross-check:` line. <!-- cold -->
