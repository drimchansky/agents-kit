...
The workflow runs roughly in order, but you don't need every step — pick whichever fits the task. The six spine skills (`explore`, `refine-idea`, `resume-task`, `plan-task`, `review-task`, `implement-plan`) are domain-neutral; the diff/review skills (`review-commit`, `review-pr`, `audit`, `verify-issue`, `review-docs`) are engineering-specific.
