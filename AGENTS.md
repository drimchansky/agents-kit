...
**The store walk.** `collect`, `nestedTaskDirs`, and `scripts/task-move.ts`'s registered-root search all classify a directory entry through `classifyWalkEntry` (§ *`scripts/lifecycle-constants.ts`*), so the prunes — `node_modules`, dotted names other than `.agents`, and a directory entry that is a link — the container-before-recognition ordering, and the stop at the first folder the recognition set claims are one implementation rather than three copies to keep in step. What is still checked by hand is **the root set**, on which the two tools diverge by design and in two places: move keeps a registered root nested inside another and searches it on its own account where this walk folds it into its container (§ *Why physical identity is settled before ambiguity is*), and move reads the canonical root one level deep where this walk recurses into it. Each divergence leaves tasks one tool sees and the other does not — a task under a nested registered root this walk never counts, a task under a canonical group no bare slug resolves — so a slug that move resolves to a folder this walk never counted, or the reverse, is checked against these two before it is treated as a defect.

**Why `nested-task` is the one check that looks inside a claimed folder.** Every walk stops at the first folder the recognition set claims, so a role file misfiled into a group — a `CONTEXT.md` where `GROUP_CONTEXT.md` belonged — swallows every task beneath it with no diagnostic. The check descends from the claim, under the same prunes, and names what it finds; it does not change where the walk stops, so slug resolution and `scanned` still see the group as the one task it has become — the finding is the only place the hidden folders appear.
...
The task constants `scripts/health-check.ts`, `scripts/task-move.ts`, and `scripts/task-state.ts` read — the plan status vocabulary, the terminal set, the compaction size trigger, the folder and record size budgets, the recognition set that identifies a task folder by its contents, and the store walk's own prunes and container names — each owned in prose by the reference § *Consumer lists* names for it, this module being their one sanctioned machine-readable copy, changed in the same edit as the prose. Left stale, a renamed status reads as `unknown` here, and the lifecycle checks that skip `unknown` — stale, done-unarchived, started-in-backlog — go quiet on every task holding it.

`classifyWalkEntry` is the exception to "constants only", and it earns it by being an **ordering** rather than a value. Every walk over a store asks the same four questions of a directory entry in the same order — prune, lifecycle container, recognition set, otherwise descend — and only the third and fourth are visible in what a walker does with the answer. A walker that keeps its own copy of that ordering can drop the container question without dropping anything a reader would miss, which is exactly what `nestedTaskDirs` did: it claimed an `Archive/` holding a misfiled role file and stopped, hiding the tasks below it. Sharing the prunes as constants would not have prevented that, because the defect was the order and not the values. The classifier reads the child directory through a caller-supplied thunk so each walker keeps its own error policy — `listEntries` warns and continues, `readOrRefuse` refuses the run — and so a pruned entry is never read at all.
...
**Why slug resolution refuses what it cannot read.** Every read that decides a slug goes through one policy: `ENOENT` and `ENOTDIR` are absence and stay silent, and any other error is concealment and exits 2 naming the path and the cause before any rename. The distinction is what the read is being asked. Whether one named folder holds a role file tolerates an empty answer; whether this slug is *unique* does not, because an unreadable directory can hold a second folder of that name and a swallowed error would move whichever match the search happened to see. Uniqueness is decided across the canonical root, the registry file, and every registered root together, so the policy covers all three rather than the deep walk alone — a registry that cannot be read hides the roots that would have held the twin exactly as an unreadable group hides the twin itself. The catch-all readers that answer an unreadable directory with an empty one (`readdirNames`, and the listing behind `isTaskFolder`) survive only outside resolution, where the question is the tolerant one. An absent root is the opposite case and only warns, since it conceals nothing and one registry legitimately describes several machines (`references/workflow/task-store.md` § *The root registry*). The bound a path argument draws (`boundingRoot`) asks the tolerant question of that same registry: it needs neither completeness nor uniqueness, only the innermost root holding the folder, so a registry the slug route refuses leaves it with no registered root and the `.agents/tasks` fallback, and it warns about nothing — a path-route refusal is one stderr line by contract, and the registry was never that route's to report.

**Why physical identity is settled before ambiguity is.** Registrations overlap — one store registered under two spellings, or a project root registered beside the store that contains it — and a folder found twice that way is one folder, not the layout error the ambiguity refusal exists to surface. Matches are keyed by their physical path, so what the refusal counts is folders rather than routes to them. That keying is what lets root collapsing stay narrow: only two spellings of one resolved directory are searched once, while a root registered inside another is searched on its own account. Folding it into its container would assume the outer walk reaches everything it holds, and the walk's own prunes break that assumption — a dotted or `node_modules` ancestor, or one holding a role file where the walk stops, leaves the inner root unreachable through the outer one and its tasks resolvable by no bare slug at all. The roots that survive that collapse are then searched in identity order rather than registry order, which the dedupe itself does not need: it buys an ambiguity refusal whose two candidate paths come out the same way on every machine whose registry holds the same roots, however each listed them. Sorting is the whole of that — drop it and the message's order becomes the registry's.

The walk's prunes and its stop at a task folder are the ones `scripts/health-check.ts` performs in `collect`; see the mirror note in that section. The fence and status halves mirror that script too.
...
- `references/workflow/task-lifecycle.md` § *Status values*, `references/workflow/status-transitions.md` § *Terminal vs. live states*, `references/workflow/reconciliation-compaction.md` § *Compaction (size trigger)*, `references/workflow/task-layout.md` § *One task, one flat folder*, `references/workflow/task-store.md` § *The root registry*, `references/workflow/task-archiving.md` § *Recognizing the directory is case-insensitive*, and `references/workflow/task-backlog.md` § *Recognizing the directory is case-insensitive* — `scripts/lifecycle-constants.ts` carries the machine-readable copy of the plan status vocabulary, the terminal set, the compaction trigger and — both owned by the fourth section — the recognition set and the folder and record size budgets, the store walk's own prunes, and the two container names, because the scripts that enforce these values cannot read prose at run time. Seven homes, eight mirror notes, one module: each home names the copy, and a change to any of those values changes the module in the same edit. The importers are the enumeration the homes point here for, given as the symbols each script actually names so the list can be diffed against its import statement:
    ...
    - `scripts/sweep-scope.ts` — `holdsRoleFile`

    The recognition set reaches both walkers through `classifyWalkEntry` rather than by name, `holdsRoleFile` excepted where three scripts test a folder directly — `scripts/task-move.ts` the folder it is asked to move, `scripts/commit-scan.ts` and `scripts/sweep-scope.ts` the task folder they are handed, the last also using it to tell a deliverable from a role file; so do the prunes — `WALK_SKIP_DIRS`, which no script imports, and `TASK_STORE_DIR`, which `scripts/task-move.ts` also imports by name for the `.agents/tasks` store bound `boundingRoot` draws, not as a prune. Kept here rather than in each home because the membership is per-constant, so no one home can state it without stating the others' as well.
