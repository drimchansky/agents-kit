...
- `scripts/` owns the repository's zero-dependency Node helpers; [SCRIPTS.md](./SCRIPTS.md) owns their CLI forms, stdout contracts, and design rationale — the sources themselves carry no comments, so a contract that changes is changed there in the same edit.
- `tests/` owns repository verification; [SCRIPTS.md](./SCRIPTS.md) § *`tests/`* names each suite, what it covers, and how to run them.
...
- the harness under `tests/` covering a script you changed — `node --test tests/setup-install.test.ts` for `setup.ts`, `node --test tests/health-check.test.ts` for `scripts/health-check.ts` and its `scripts/lifecycle-constants.ts` import, `node --test tests/task-move.test.ts` for `scripts/task-move.ts` and `node --test tests/task-state.test.ts` for `scripts/task-state.ts`, which share that import, `node --test tests/cross-capability.test.ts` for `scripts/cross-capability.ts`, `node --test tests/session-triage.test.ts` for `scripts/session-triage.ts`, `node --test tests/pr-comments.test.ts` for `scripts/pr-comments.ts`, `node --test tests/size-report.test.ts` for `scripts/size-report.ts`, `node --test tests/size-check.test.ts` for `scripts/size-check.ts`, `node --test tests/worktree-merge.test.ts` for `scripts/worktree-merge.ts`;
- [SCRIPTS.md](./SCRIPTS.md) when changing `setup.ts`'s or a script's CLI form, its stdout or exit contract, what a `tests/` suite covers, or the reasoning a caller depends on — the `.ts` sources carry no comments, so that file is the only place those live, and the skills that run a script cite its section by path;
