...
Cite the friction motivating a kit change: a `~/.local/state/agents-kit/session-findings-*.md` finding or a consumer-project task where it occurred. A failing test is the sole exemption; cite the failure. Without either, defer the addition. The duplicate scan catches repetition, not unsupported growth; spend the friction citation when adding corpus prose.
