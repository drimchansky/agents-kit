...
- **Engineering-workflow skills** — operate on code and participate in the understand → plan → implement → review → verify → document loop. They carry the shared rules contract (symlink + directive, see below). Today, in workflow order: `explore`, `refine-idea`, `resume-task`, `plan-task`, `review-task`, `implement-plan`, `review-commit`, `review-pr`, `audit`, `verify-issue`, `review-docs`. Preserve the workflow ordering when listing them in docs and when inserting new ones (e.g. a new review-stage skill goes near `review-pr`, not at the alphabetical end).
...
- `references/` is partitioned by domain. Today two subdirectories exist: `references/engineering/` holds technical/code-domain checklists (TypeScript, React, CSS, tanstack-query, security, performance, accessibility, testing, code-style), and `references/workflow/` holds methodology checklists consulted by orchestration skills (acceptance-criteria, task-lifecycle, review). Future domains (e.g. prose, design) can live as sibling subdirectories. Every engineering skill carries the same `## References` block that says "read any applicable checklists from `references/engineering/`", so adding a new engineering checklist requires no per-skill change — it picks up automatically. Workflow checklists are pulled in via **direct citation** at the specific use sites that need them (e.g. `plan-task` and `review-task` cite `references/workflow/acceptance-criteria.md` directly; `review-commit`, `review-pr`, and `audit` cite `references/workflow/review.md`; the lifecycle-bearing skills cite `references/workflow/task-lifecycle.md`) — they do **not** participate in the generic engineering auto-pickup. Standalone skills (`proofread`, `translate`, `fact-check`) intentionally omit the References block: code-domain checklists don't apply to their work, and there's no prose-domain references subdirectory yet. When you add a new domain, mirror the engineering convention: a per-domain subdirectory and a matching `## References` block in the skills that need it.
